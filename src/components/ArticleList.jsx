  ...
  return (
    <div className={styles.articleList}>
      <header className={styles.header}>
        <h2>{isLoading ? <Skeleton width={150} /> : (selectedFeed ? selectedFeed.title : 'Articles')}</h2>
      </header>
        ...
        {isLoading ? (
          Array.from({ length: 5 }).map((_, index) => (
            <div className={styles.articleCard} key={index}>
              ...
              <p className={styles.cardSnippet}><Skeleton count={2} /></p>
            </div>
          ))
        ) : (
          articles.map((article, index) => (
            <div
              ...
              className={`${styles.articleCard} ${selectedArticle?.link === article.link ? styles.active : ''}`}
            >
